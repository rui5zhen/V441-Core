...
# V441-Core: 441-Layer High-Precision UAV Object Detector（欢迎来到5er的快乐仓库）


<p align="center">
  <img src="https://img.shields.io/badge/Python-3.10%2B-blue.svg" alt="Python">
  <img src="https://img.shields.io/badge/SOTA-VisDrone%20mAP%200.528-red.svg" alt="SOTA">
...
本实验使用 **VisDrone-DET** 数据集，已为您完成预处理（包含图像和标注文件，并已按 YOLO 格式整理）。请通过以下链接下载：

- **百度网盘**：[点击下载](https://pan.baidu.com/s/1ZCAwqPZagZ3b6iuUJ7BbmQ?pwd=utj3)  
  提取码：`utj3`

下载后解压到项目根目录下的 `datasets/` 文件夹中，确保目录结构如下：
```
datasets/
└── VisDrone_DET/
    ├── images/
    ...
    │   ├── train/
    │   └── val/
    └── VisDrone_DET.yaml   # 数据集配置文件
```

仓库中已提供 `VisDrone_DET.yaml` 配置文件（位于根目录），其中已正确设置路径。如果您将数据集放在其他位置，请修改该文件中的 `path`、`train`、`val` 字段。
...
| `--model` | 模型定义文件路径 | 必需 |
| `--epochs` | 训练轮数 | 300 |
| `--batch` | 每卡批次大小（24GB显存建议8） | 8 |
| `--device` | GPU编号 | 0 |
| `--lr0` | 初始学习率 | 0.0002 |
| `--amp` | 是否开启混合精度（必须关闭） | False |
...
| **数据集路径错误** | 未正确配置 `VisDrone_DET.yaml` 中的路径 | 检查 yaml 文件中的 `path`、`train`、`val` 路径是否正确 |
...
**如果您在复现过程中遇到任何问题，欢迎提交 Issue。** 祝您复现顺利，天天开心
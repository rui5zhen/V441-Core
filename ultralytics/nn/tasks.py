...
# --- 强制注入 Mamba 进化版模块 ---
try:
    from .modules.mamba_modules import VSSBlock, LG_VSSBlock, OctoLG_VSSBlock 
    globals()['VSSBlock'] = VSSBlock
    globals()['LG_VSSBlock'] = LG_VSSBlock
    globals()['OctoLG_VSSBlock'] = OctoLG_VSSBlock 
    print("✅ [Mamba Evolution] LG_VSSBlock & OctoLG_VSSBlock 已成功注入全局命名空间")
except ImportError as e:
    print(f"❌ [Mamba Evolution] 导入失败: {e}")

from ultralytics.nn.modules import (
    AIFI, C1, C2, C3, C3TR, OBB, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C2fAttn,
    ImagePoolingAttn, C3Ghost, C3x, Classify, Concat, Conv, Conv2, ConvTranspose,
    Detect, DWConv, DWConvTranspose2d, Focus, GhostBottleneck, GhostConv, HGBlock,
    HGStem, Pose, RepC3, RepConv, ResNetLayer, RTDETRDecoder, Segment, WorldDetect,
    RepNCSPELAN4, ADown, SPPELAN, CBFuse, CBLinear, Silence, C2fCIB, PSA, SCDown,
    RepVGGDW, v10Detect, VSSBlock
)
...
from ultralytics.utils.torch_utils import (
    fuse_conv_and_bn, fuse_deconv_and_bn, initialize_weights, intersect_dicts,
        ...
        if not self.is_fused():
            for m in self.model.modules():
                if isinstance(m, (Conv, Conv2, DWConv)) and hasattr(m, "bn"):
                    if isinstance(m, Conv2): m.fuse_convs()
...
class DetectionModel(BaseModel):
    ...
    # ⭐ 核心修复点：让 DetectionModel 自动识别 YOLOv10 Loss
    def init_criterion(self):
    ...
    def _predict_augment(self, x):
        ...
        for si, fi in zip(s, f):
            ...
            yi = super().predict(xi)
            if isinstance(yi, dict): yi = yi["one2one"]
            if isinstance(yi, (list, tuple)): yi = yi[0]
    ...
    def _descale_pred(p, flips, scale, img_size, dim=1):
        ...
        if flips == 2: y = img_size[0] - y
        elif flips == 3: x = img_size[1] - x
...
def parse_model(d, ch, verbose=True):
    ...
    if scales:
        scale = d.get("scale", tuple(scales.keys())[0] if isinstance(scales, dict) else 0)
        scale_values = scales[scale] if isinstance(scales, dict) else scales
        depth, width = scale_values[0], scale_values[1]
        if len(scale_values) > 2: max_channels = scale_values[2]

    ch = [ch]
    layers, save, c2 = [], [], ch[-1]
    
    for i, (f, n, m_str, args) in enumerate(d["backbone"] + d["head"]):
        m = getattr(torch.nn, m_str[3:]) if "nn." in m_str else globals()[m_str]
        ...
        def get_ch(x):
            try:
                return ch[x] if x < 0 else ch[x + 1]
            except IndexError:
                return ch[x] 

        # --- A. 识别需要缩放通道的模块 (加入 OctoLG_VSSBlock) ---
        if m in {Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, Focus, 
                BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, PSA, SCDown, C2fCIB, 
                RepNCSPELAN4, ADown, SPPELAN, VSSBlock, LG_VSSBlock, OctoLG_VSSBlock}:
            ...
            # --- B. 识别需要插入重复参数 n 的模块 (加入 OctoLG_VSSBlock) ---
            if m in (BottleneckCSP, C1, C2, C2f, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB, VSSBlock, LG_VSSBlock, OctoLG_VSSBlock):
                args.insert(2, n)
                n = 1
